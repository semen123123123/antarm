...
router.get('/', async (req, res) => {
  ...
  let reviews;
  if (activeOnly) {
    reviews = await db.prepare('SELECT * FROM video_reviews WHERE is_active = 1 ORDER BY sort_order ASC, created_at DESC').all();
  } else {
    reviews = await db.prepare('SELECT * FROM video_reviews ORDER BY sort_order ASC, created_at DESC').all();
  }
  res.json(reviews);
});
...
router.get('/:id', async (req, res) => {
  const db = getDb();
  const review = await db.prepare('SELECT * FROM video_reviews WHERE id = ?').get(req.params.id);
...
router.post('/', async (req, res) => {
  ...
  const review = await db.prepare('SELECT * FROM video_reviews WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', async (req, res) => {
  ...
  const existing = await db.prepare('SELECT * FROM video_reviews WHERE id = ?').get(req.params.id);
  ...
  const updated = await db.prepare('SELECT * FROM video_reviews WHERE id = ?').get(req.params.id);
  res.json(updated);
});
...
router.delete('/:id', async (req, res) => {
  const db = getDb();
  await db.prepare('DELETE FROM video_reviews WHERE id = ?').run(req.params.id);
